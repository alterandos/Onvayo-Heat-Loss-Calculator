...
class BuildingElement:
    def __init__(self, data: dict):
        ...
        self.delta__utb = data['delta__utb']
        self.theta__int_build = data['theta__int_build']
    ...
    def calculate_simplified_be_design_transmission_loss(self) -> float:
        return BuildingElement.calculate_simplified_be_design_transmission_loss(
            self.a__k, self.u__k, self.delta__utb, self.f__x, self.theta__int_build, self.theta__e)


    @staticmethod
    def calculate_simplified_be_design_transmission_loss(
    ...
    def get_simplified_thermal_transmittance_u(self):
        return BuildingElement.get_simplified_thermal_transmittance_u(self.be_type, self.be_sub_type, self.build_year)
    ...
    def get_simplified_temperature_adjustment_term(self):
        return BuildingElement.get_simplified_temperature_adjustment_term(self.be_adjacent_to)
...
class Building:
    def __init__(self,
        ...
        self.delta__utb = self.delta__utb if self.delta__utb is not None else self.get_simplified_additional_thermal_transmittance_for_thermal_bridges(self.delta__utb_selection_criteria)
        # Air change rate in h-1
        self.n__build = self.n__build if self.n__build is not None else self.get_simplified_air_change_rate(self.build_year, self.air_tightness_level)
        # Internal design temperature of the considered heated building in °C
        self.theta__int_build = self.theta__int_build if self.theta__int_build is not None else self.get_simplified_internal_design_temperature(self.building_type)
    ...
    def get_simplified_additional_thermal_transmittance_for_thermal_bridges(delta__utb_selection_criteria:str=None):
        ...
        if delta__utb is None:
            delta__utb = din_12831_data.b_3_2_simplified_thermal_bridges() if delta__utb_selection_criteria is None else din_12831_data.b_2_1_simplified_thermal_bridges(delta__utb_selection_criteria)
        return delta__utb
